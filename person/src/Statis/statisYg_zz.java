...
public class statisYg_zz extends JFrame {
	public statisYg_zz() {
		...
		JFrame framestaff = new JFrame("在职员工");
		...
		try {
			...
			String sql = "select * from 在职教师 union select * from 教师信息 where 在职信息='在职'";
			// 获取结果集元数1
			...
			final JFrame f = new JFrame("在职教师");
...
/*			TableColumn column = null;
			...
			columnName.add("在职信息");
			columnName.add("部门编号");
			new DBtoExcel().WriteExcel(rs, "E://statisYg_zz.xls", "在职教师", columnName);
							...
							public void actionPerformed(ActionEvent e) {
								...
								jb1.addActionListener(new ActionListener() {
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												String sql = "SELECT * FROM 在职教师  WHERE 性别 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 性别 LIKE" + "'%" + c1 + "%'"+" and 在职信息='在职'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
												...
												final JFrame f = new JFrame("在职教师统计结果");
									...
									/*		TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_zz_xb.xls", "教师信息_性别",
														columnName);
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 在职教师  WHERE 学历 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 学历 LIKE" + "'%" + c1 + "%'"+" and 在职信息='在职'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
										...
										/*		TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_zz_xl.xls", "教师信息_学历",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 在职教师  WHERE 职务 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 职务 LIKE" + "'%" + c1 + "%'"+" and 在职信息='在职'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
											...
											/*	TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_zz_zw.xls", "教师信息_职务",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 在职教师  WHERE 职称 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 职称 LIKE" + "'%" + c1 + "%'"+" and 在职信息='在职'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
											...
											/*	TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_zz_zc.xls", "教师信息_职称",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 在职教师  WHERE 部门编号 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 部门编号 LIKE" + "'%" + c1 + "%'"+" and 在职信息='在职'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
											...
											/*	TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_zz_bmbh.xls", "教师信息_部门编号",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
