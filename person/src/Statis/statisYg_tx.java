...
public class statisYg_tx extends JFrame {
	public statisYg_tx() {
		...
		JFrame framestaff = new JFrame("退休员工");
		...
		try {
			...
			String sql = "select * from 退休教师 union select * from 教师信息 where 在职信息='退休'";
			// 获取结果集元数1
			...
			final JFrame f = new JFrame("退休教师");
		...
		/*	TableColumn column = null;
			...
			columnName.add("在职信息");
			columnName.add("部门编号");
			new DBtoExcel().WriteExcel(rs, "E://statisYg_tx.xls", "退休教师", columnName);
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												String sql = "SELECT * FROM 退休教师  WHERE 性别 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 性别 LIKE" + "'%" + c1 + "%'"+" and 在职信息='退休'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
												...
												final JFrame f = new JFrame("退休教师统计结果");
											...
											/*	TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_tx_xb.xls", "教师信息_性别",
														columnName);
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 退休教师  WHERE 学历 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 学历 LIKE" + "'%" + c1 + "%'"+" and 在职信息='退休'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
											...
											/*	TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_tx_xl.xls", "教师信息_学历",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 退休教师  WHERE 职务 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 职务 LIKE" + "'%" + c1 + "%'"+" and 在职信息='退休'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
										...
										/*		TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_tx_zw.xls", "教师信息_职务",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 退休教师  WHERE 职称 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 职称 LIKE" + "'%" + c1 + "%'"+" and 在职信息='退休'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
										...
										/*		TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_tx_zc.xls", "教师信息_职称",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (c1.equals("")) {
											...
											try {
												...
												conn.setAutoCommit(true);
												System.out.println("已经连接到数据库...");
												String sql = "SELECT * FROM 退休教师  WHERE 部门编号 LIKE" + "'%" + c1 + "%'"+" union SELECT * FROM 教师信息  WHERE 部门编号 LIKE" + "'%" + c1 + "%'"+" and 在职信息='退休'";
												st = conn.createStatement();
												rs = st.executeQuery(sql);
												int count = 0;
												while (rs.next()) {
											...
											/*	TableColumn column = null;
												...
												columnName.add("在职信息");
												columnName.add("部门编号");
												new DBtoExcel().WriteExcel(rs, "E://statisYg_tx_bmbh.xls", "教师信息_部门编号",
														columnName);
											} catch (SQLException e1) {
												java.lang.System.out.println("异常" + e1);
											} finally {
